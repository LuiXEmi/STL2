...
void R34(){
    ...
    expresion = e2->DevuelveNodo();
    coma = e3->getElementos();
    ObR34 *nr4 = new ObR34(coma,expresion,listaargumentos);
    NoTerminal *nt = new NoTerminal(nr4);
    fila = p1.top();
    dato = fila->getElemento();
    int columna = ListaArgumentos;
    int elemento = Matriz[dato][columna];
...
bool nAnalizadorS(int operando,string cadena){
    ...
    cout<<"ELEMENTO: "<<elemento<<endl<<endl;
    if(elemento > 0){
...
void PilaMuestra(){
    ...
    if (e1->esNoTerminal()==true){
        cout<<endl<<"------ARBOL SINTACTICO-------"<<endl;
        e1->muestra();
    }

    cout<<endl<<"---------SEMANTICO-------"<<endl;
    e1->semantica();
    e1->usadas();
...
int main(){
    loadfile();
    StoreMatriz();
    //string cadena = "int main ( ) { float a ; int b ; int c ; c = a + b ; c = suma ( 8.5 , 9.9 ) ; } $";
    string cadena = "int main ( ) { int a ; string d ; int b ; int c ; c = a + b ; } $";
    Estado *t1;
    Estado *t2;
    t1 = new Estado(23);
    t2 = new Estado(0);
